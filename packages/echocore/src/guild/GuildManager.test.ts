...
import { GuildTask, GuildResult, GuildEvent } from './GuildContract';
...
class TestGuildMember extends BaseGuildMember {
  public taskHistory: GuildTask[] = [];
  public eventHistory: GuildEvent[] = [];
  ...
  public setFailNextTask(shouldFail: boolean): void {
    this.executeTask = async (task: GuildTask): Promise<GuildResult> => {
      this.taskHistory.push(task);
...
describe('GuildManager', () => {
  ...
  describe('Task Dispatching', () => {
    let worker1: TestGuildMember;
    let worker2: TestGuildMember;
    let manager: TestGuildMember;
