 ...
 */
function createAgentWithMailbox(agentId, messageBroker) {
  // Create the agent
 ...
 */
function setupMessageHandlers(agentA, agentB, agentC) {
  ...
  const announcementHandler = (agent) => {
    return (message) => {
 ...
 * @param agentC The third agent
 */
async function sendDirectMessages(agentA, agentB, agentC) {
 ...
 * @param agentB The second agent
 */
async function broadcastMessages(agentA, agentB) {
 ...
 */
async function queryMessages(agentC) {
  console.log('\n🔍 Querying messages...');
...
//# sourceMappingURL=messaging-example.js.map
