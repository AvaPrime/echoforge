...
const SKIP_CREDENTIALS = process.env.SKIP_CREDENTIALS_LOADING === 'true';
...
async function loadCredentials() {
  if (SKIP_CREDENTIALS) {
    Logger.info('Skipping credentials loading (development mode)');
    return null;
  }
...
async function startServer() {
  try {
    ...
    app.listen(PORT, () => {
      ...
      Logger.info(`🔐 Credentials loading: ${SKIP_CREDENTIALS ? 'SKIPPED' : 'ENABLED'}`);
    });
  } catch (error) {
